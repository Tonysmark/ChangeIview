					...
					<a-form-model :label-col="{ span: 2 }" :wrapper-col="{ span: 22 }" ref="headlineForm" :model="headlineForm" :rules="headlineRule">
						...
						<a-form-model-item label="头条封面：" required>
							<div class="upload-container type-img" style="width: 50%">
								...
								<input type="file" title="点击选择图片" accept="image/jpg,image/jpeg,image/png,image/gif" @change="handleCoverChange($event)" />
							</div>
						</a-form-model-item>
						...
						<a-form-model-item label="头条生效期间：">
							<DatePicker
								style="width: 16em"
								type="daterange"
								:value="[headlineForm.validityDateStart, headlineForm.validityDateEnd]"
								placement="top-start"
						...
						<a-form-model-item label="头条视频：" :required="headlineForm.headlineProperty == 1">
							<div class="flex flex-middle">
								...
								<Progress class="flex-1 margin-right" v-if="headlineVideo.percent > 0 && headlineVideo.percent < 100" :percent="headlineVideo.percent"></Progress>
								...
								<template v-if="headlineVideo.percent == 0 || headlineVideo.percent == 100">
									<span class="ivu-upload margin-right">
										...
										<input ref="headlineVideo" type="file" accept="video/mp4" data-max-size="500" @change="handleFileChange($event, 'headlineVideo')" />
									</span>
						...
						<template v-if="headlineForm.headlineProperty == 2">
							<a-form-model-item label="头条音频：">
								<div class="flex flex-middle">
									...
									<Progress class="flex-1 margin-right" v-if="headlineVoice.percent > 0 && headlineVoice.percent < 100" :percent="headlineVoice.percent"></Progress>
									...
									<template v-if="headlineVoice.percent == 0 || headlineVoice.percent == 100">
										<span class="ivu-upload margin-right">
											...
											<input ref="headlineVoice" type="file" accept="audio/mp3,audio/wav,audio/mpeg,audio/x-wav" data-max-size="500" @change="handleFileChange($event, 'headlineVoice')" />
										</span>
							...
							<a-form-model-item label="头条附件：">
								<div class="ivu-upload">
									...
									<input
										...
										type="file"
										multiple
										accept="image/jpg,image/jpeg,image/png,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/vnd.ms-powerpoint,application/vnd.openxmlformats-officedocument.presentationml.presentation,application/pdf"
								...
								<template v-if="!!attachments.length">
									<div v-for="(item, index) in attachments" :key="item.timestamp" class="flex flex-middle">
										...
										<Progress class="flex-1" v-if="item.percent > 0 && item.percent < 100" :percent="item.percent"></Progress>
